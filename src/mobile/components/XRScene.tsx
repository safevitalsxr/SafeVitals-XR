...
export function XRScene({ scrollProgress }: SceneProps) {
  // Scene 9 Range: 0.78 -> 0.92
  const sceneOpacity = useTransform(scrollProgress, [0.78, 0.81, 0.88, 0.92], [0, 1, 1, 0]);
  const scale = useTransform(scrollProgress, [0.78, 0.81, 0.88, 0.92], [0.94, 1, 1, 0.94]);
  const y = useTransform(scrollProgress, [0.78, 0.81, 0.88, 0.92], [30, 0, 0, -30]);
  ...
  // Text 1: "See beyond numbers." (0.79 -> 0.86)
  const text1Opacity = useTransform(scrollProgress, [0.78, 0.81, 0.84, 0.86], [0, 1, 1, 0]);
  const text1Y = useTransform(scrollProgress, [0.78, 0.81, 0.84, 0.86], [15, 0, 0, -15]);

  // Text 2: "See the patient." (0.86 -> 0.92)
  const text2Opacity = useTransform(scrollProgress, [0.85, 0.88, 0.90, 0.92], [0, 1, 1, 0]);
  const text2Y = useTransform(scrollProgress, [0.85, 0.88, 0.90, 0.92], [15, 0, 0, -15]);
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-grow flex items-center justify-center">
        ...
        <svg
          className="w-full h-full max-h-[220px] z-10 select-none opacity-40"
          viewBox="0 0 200 200"
          fill="none"
        >
          <path
        ...
        <div className="absolute inset-0 w-full h-full z-20">
          ...
          <motion.div
            className="absolute left-[5%] top-[25%] bg-[#0A1221]/70 border border-[#00D4FF]/30 p-2.5 rounded-xl backdrop-blur-md flex flex-col justify-between shadow-[0_10px_25px_rgba(0,212,255,0.08)] select-none"
            animate={{ y: [0, -6, 0] }}
            transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut" }}
          >
            <div className="flex items-center gap-1">
              <Heart className="w-3 h-3 text-[#00D4FF] animate-pulse" />
              <span className="text-[7px] font-mono text-white/40 tracking-wider">HR SPATIAL</span>
            </div>
          ...
          <motion.div
            className="absolute right-[5%] top-[30%] bg-[#0A1221]/70 border border-white/15 p-2.5 rounded-xl backdrop-blur-md flex flex-col justify-between shadow-[0_10px_25px_rgba(255,255,255,0.05)] select-none"
            animate={{ y: [0, 6, 0] }}
            transition={{ duration: 4.2, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
          >
            <div className="flex items-center gap-1">
              <Target className="w-3 h-3 text-white/50" />
              <span className="text-[7px] font-mono text-white/40 tracking-wider">RESP TETHER</span>
            </div>
            <div className="flex items-baseline gap-1 mt-1">
          ...
          <div className="absolute left-[38%] bottom-[20%] flex items-center justify-center pointer-events-none">
            ...
            <span className="text-[7px] font-mono text-[#00D4FF] ml-2 tracking-wider whitespace-nowrap bg-black/40 px-1 py-0.5 rounded border border-cyan-500/10">
              PINCH ROTATE ACTIVE
            </span>
          </div>
        </div>
