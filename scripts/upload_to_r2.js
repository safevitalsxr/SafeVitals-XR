...
const LOCAL_DIR = path.join(__dirname, '..', 'public', 'hero-xr');

const CONCURRENCY = 5; // Upload 5 files at a time

// Helper to make PUT request for R2 upload
...
async function run() {
  console.log(`Starting upload of images from ${LOCAL_DIR} to Cloudflare R2 bucket "${BUCKET_NAME}"...`);
  ...
  const files = fs.readdirSync(LOCAL_DIR)
    ...
    .sort(); // Ensure order ezgif-frame-001.png, ezgif-frame-002.png, ...

  const totalFiles = files.length;
  console.log(`Found ${totalFiles} frames to upload.`);

  let completed = 0;
  let failed = [];

  // Simple async pool for concurrency
  const queue = [...files];
  const activeWorkers = [];
    ...
    while (queue.length > 0) {
      ...
      while (attempts > 0 && !success) {
        try {
          ...
          console.log(`[Progress] Uploaded ${file} successfully (${completed}/${totalFiles} - ${percent}%)`);
        } catch (err) {
          attempts--;
